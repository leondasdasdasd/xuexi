...
class MultiClassTable extends PureComponent {
    ...
    if (key == "questionNo") {
      this.props
        ...
        .then(() => {
          ...
          this.state.highScoreRateList.map((item, index) => {
            if (item.index == record.index) {
              newList.push({
                index: item.index,
  ...
  addHighScoringList = () => {
    const addObject = {
      index: this.state.highScoreRateList.length + 1,
      propositionAnalysis: "",
      questionNo: "",
      teacherAnalysis: "",
      questionScoreRate: "",
    ...
    this.setState({
      highScoreRateList: [...this.state.highScoreRateList, addObject],
    });
  };
  addLowScoringList = () => {
    const addObject = {
  ...
  scoringHighDel = (index) => {
    let array = JSON.parse(JSON.stringify(this.state.highScoreRateList));
    if (array.length > 3) {
      let newArray = array.filter((item) => item.index !== index);
      this.setState({
        highScoreRateList: newArray,
      });
    }
  };
  scoringLowDel = (index) => {
      ...
      onSuccess: (res) => {
        ...
        if (res.content.modelKey == 3) {
          this.setState({
            ...
            highScoreRateList: res.content.highScoreRateList,
    ...
    return (
      ...
      >
        ...
        <div className={styles.classTab}>
          {this.props?.childModuleConfig?.map((componentConfig) => {
            if (
              componentConfig.moduleKey == "analysisHighestScoring" &&
              componentConfig.moduleShow
            ) {
              return highScoreRateList == undefined ? null : (
                <div
              ...
              componentConfig.moduleKey == "analysisLowestScoring" &&
              componentConfig.moduleShow
            ) {
              return lowScoreRateList == undefined ? null : (
                <div
                  className={styles.lowestScoring}
                  style={{ marginBottom: "12px" }}
                >
                  <div className={styles.classTabTitle}>
                    {/* <Icon type="caret-right" /> */}
                    {/* {trans("global.analysisLowestScoring", "得分率最低的题目分析")} */}
                    {locale() == "en"
                      ? componentConfig.moduleEname
                        ? componentConfig.moduleEname
                        : componentConfig.moduleName
                  ...
                  <Table
                    columns={lowScoreRateClumns}
                    dataSource={lowScoreRateList}
                    bordered={true}
                    pagination={false}
                  />
                  {lowScoreRateEdit ? (
                    <div className={styles.increaseRow}>
                      <div
                        className={styles.increase}
                        onClick={this.addLowScoringList}
                      >
                        <i
                          className={icon.iconfont}
                          style={{
              ...
              componentConfig.moduleKey == "differentiationAnalysis" &&
              componentConfig.moduleShow
            ) {
              return (
              ...
              componentConfig.moduleKey == "classText" &&
              componentConfig.moduleShow
            ) {
              return this.props.gradeName &&
                ...
                >
                  ...
                  {this.state.classEdit && this.state.classEdit1 ? (
                    <>
                      <RicherEditor
                    ...
                    <div
                      ...
                      style={{ padding: 5 }}
                    >
                      {this.state.classEdit1 ? null : (
                        <div
                          onClick={this.classEdit1Change}
                          style={{
                            position: "absolute",
                            top: "0",
                            left: "0",
                      ...
                      {this.state.classAnalysisText ? (
                        <>
                          <div
                            ...
                            // onMouseLeave={() => this.setState({ editFont2: false })}
                            // onMouseOver={() => this.setState({ editFont2: true })}
                        ...
                        <>
                          ...
                          >
                            {locale() == "en"
                              ? componentConfig.defaultEmessage
                                ? componentConfig.defaultEmessage
                                : componentConfig.defaultMessage
                              : componentConfig.defaultMessage}
                            {/* {trans(
