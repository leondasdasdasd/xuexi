...
class LeftContent extends React.Component {
  ...
  render() {
    const { trendStuList, studentUserId, groupId, classListData } = this.props;
        ...
        <div style={{ flexGrow: "1", marginTop: "10px" }}>
          {trendStuList && trendStuList.length > 0
            ? trendStuList.map((item) => (
                <div
                  key={item.studentId}
                  className={`${styles.userBox} ${studentUserId === item.studentId ? styles.isChecked : ""}`}
                  onClick={() => this.changeStu(item.studentId)}
                >
                  <div className={[styles.nameBox].join(" ")}>
                    <div>
                      {locale() == "en" ? item.enName : item.studentName}
                    </div>
                  </div>
                </div>
              ))
