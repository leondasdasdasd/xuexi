...
class RealTime extends PureComponent {
  ...
  renderChart = () => {
    ...
    let newData = [];
    let rightObject = {
      name: trans("global.right", "正确"),
    };
    let wrongObject = {
      name: trans("global.wrong", "错误"),
    };
    let newArray = [];
    let newWrong = [];
    let newNoReply = [];
    ...
    if (
      ...
      viewChart.questionModels.map((item) => {
        const questionLabel = `${trans("realTime.questionPrefix", "题目")}${item.questionSerialNumber}`;
        ...
        questionList.push(questionLabel);
        newArray.push({
          name: trans("global.right", "正确"),
          year: questionLabel,
        ...
        newNoReply.push({
          name: trans("analysisStudent.notAnswered", "未作答"),
          year: questionLabel,
          questionSerialNumber: item.questionSerialNumber,
          value: item.questionNotAnswerRate || 0,
          number: item.questionNotAnswerNumber || 0,
          className: "noReply",
        });
      });
    }
    newData.push(rightObject, wrongObject);
    newData = [...newArray, ...newWrong, ...newNoReply];
    // newData = newArray.concat(newNoReply);
    ...
    chart.tooltip({
      ...
      itemTpl: `<li>
              ...
              <span style='margin-left: 10px;'>{number}</span>${trans("global.peopleUnit", "人")}
    ...
    chart
      ...
      .color("name", ["#0B9C37", "#9C0B11", "#FFE030"])
      .tooltip(
    ...
    chart.on("interval:click", (event_) => {
      ...
      const questionNumber = newData._origin.questionSerialNumber;
      let id = null;
      viewChart.questionModels.map((item) => {
    ...
    return (
      <div className={styles.chartDiv}>
        {/* <div className={styles.header}>
          <i className={[styles.iconfont, styles.backIcon].join(' ')} onClick={this.back}>&#xe6ff;</i>
          <span className={styles.title}>{trans('global.realTimeData', '实时统计数据')}</span>
        </div> */}
