...
class KnowledgePoint extends PureComponent {
    ...
    this.props.dispatch({
      ...
      onSuccess: (res) => {
        if (res.content) {
          this.setState(
            {
              dataSourceObj: res.content,
            },
            () => {
              if (this.state.check == 1) {
                this.handelTableData();
                this.handelTableColumns();
  ...
  handelTableData = (data) => {
    ...
    let tableDataSource = dataSourceObj || {};

    let newDataSource = [];

    // 这里根据viewType来区分行与列 2为班级视角，1为知识点视角 （班级视角下行数据为班级，列数据为知识点，知识点视角同理）
    ...
    this.setState({
      knowledgePointTableData: newDataSource,
    });
  ...
  handelTableColumns = () => {
    ...
    if (this.props.viewType == 2) {
              ...
              return ind === 0 ? (
                ...
                <div>
                  ...
                  <span
                    className={[
                      styles.publicMessage,
                      comparePercentages(txt, record["1ScoreRate"]) == -1
                        ? styles.noPass
                        : "",
                    ].join(" ")}
                  >
      ...
      newColumns.splice(1, 0, {
        title: trans("global.questionParallel", "对应题号"),
    ...
    this.setState({
      knowledgePointTableColumns: newColumns,
    });
  };

  initRadarChart = () => {
    const { dataSourceObj: dataSourceObject = {} } = this.state;
    const { qualityIndicatorData, columnSet } = dataSourceObject || {};
    ...
    list?.map((item, index) => {
      ...
      if ((this.state.viewType == 2 && index > 1) || this.state.viewType == 1) {
        ...
        if (!document.getElementById(`knowledgeRadar${index}`)) {
          return;
        }
        ...
        let chartHeight = 400;
        let radius = 0.7;
        let padding = [0, 30, 0, 30];
        let formatter = (value) => {
          // 超过8个字就断行，每8个字符加一个换行
          return value.length > 8 ? value.replaceAll(/(.{8})/g, "$1\n") : value;
        };
        ...
        if (data.length > 60) {
          ...
          chartHeight = 400;
          radius = 0.5;
          formatter = (value, item, labelIndex) => {
            return labelIndex % 3 == 0 ? value : ""; // 隔一个显示一个
          };
          padding = [0, 0, 0, 0];
          ...
          radius = 0.5;
          formatter = (value, item, labelIndex) => {
            return labelIndex % 2 == 0 ? value : ""; // 隔一个显示一个
          };
          padding = [0, 0, 0, 0];
        }

        this[`knowledgeRadarChart${index}`] = new Chart({
          container: `knowledgeRadar${index}`,
          width: chartWidth,
          height: chartHeight,
          // forceFit: true,
          padding: padding,
  ...
  renderChart = () => {
    ...
    if (this.state.viewType == 2) {
      dataSourceObj?.qualityIndicatorData?.length &&
        dataSourceObj.qualityIndicatorData.map((item, index) => {
          seriesArray.push({
            name: item.groupName,
            type: "line",
            data: [],
      ...
      if (dataSourceObj?.columnSet?.length) {
        dataSourceObj.columnSet.map((item, index) => {
          if (index == 0) return;
      ...
      dataSourceObj?.columnSet?.length &&
        dataSourceObj.columnSet.map((item, index) => {
          if (index == 0) return;
          seriesArray.push({
            name: item.columnName,
      ...
      if (dataSourceObj?.qualityIndicatorData?.length) {
        dataSourceObj.qualityIndicatorData.map((item, index) => {
    ...
    let ele = document.querySelector("#knowledgeAnalysisChart");
    if (!ele) {
      return;
    }
          ...
          {locale() == "en" ? (
            canGoToConfigData ? (
              <span>
                ...
                ,then mark each question&apos;s knowledge point under the
                &apos;Knowledge Point&apos; column to generate analysis charts.
              </span>
            ) : (
  ...
  render() {
    const { check, dataSourceObj } = this.state;
    const { qualityIndicatorData, columnSet } = dataSourceObj || {};
    ...
    const url = `${window.location.origin}/api/export/exam/knowledge/point?examId=${this.props.examId}&type=知识点&filterFlag=${this.state.newDimensionAnalysisSpecify}`;
    return (
      <div className={styles.questionTable} id="table3">
        ...
        >
          <AreaHeaderComponent
            ...
            title={trans("analysis.knowledgeAnalysis", "知识点分析")}
            leftPanelContent={
              <>
                <MyTabs
                  data={[
          ...
          <div className={[styles.tableBoxContent].join(" ")}>
            {this.state.check == 1 ? (
              <>
                {dataSourceObj?.qualityIndicatorData?.length ? (
                  <Table
                    dataSource={this.state.knowledgePointTableData}
                    pagination={false}
                    scroll={{ x: 900 }}
                    columns={this.state.knowledgePointTableColumns}
                    key={tableKey}
                  />
                ) : (
                  this.noDataTemplate()
            ...
            {this.state.check == 2 ? (
              dataSourceObj?.qualityIndicatorData?.length ? (
                <div
                  key="knowledgeAnalysisChart"
                  id="knowledgeAnalysisChart"
                  style={{ height: 300 }}
                />
              ) : (
                this.noDataTemplate()
                ...
                {knowledgeRadarList?.length
                  ? knowledgeRadarList.map((item, key) => {
                      if (
                        ...
                        return <div id={`knowledgeRadar${key}`}></div>;
                      }
                    })
                  : this.noDataTemplate()}
              </div>
