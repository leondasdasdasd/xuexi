...
(function (expose) {
  ...
  var mk_block = (Markdown.mk_block = function (block, trail, line) {
    ...
    // 如果每个块域的第二个字符是小数点，则将该块域理解为选择题的选项
    if (s.substr(1, 1) == "." || s.substr(1, 1) == "、") {
      // 这里是将选择题的块域首字母大写
  ...
  Markdown.dialects.Gruber = {
    block: {
      ...
      qtAnswer: function qtAnswer(block, next) {
        block = block
          .replace(/^\s*【\s*答案\s*】\s*/, "答案Answer：")
          .replace(/^\s*(答案[:：])\s*/, "答案Answer：")
          .replace(/^\s*(Answer[:：])\s*/, "答案Answer：")
          .replace(/^\s*(answer[:：])\s*/, "答案Answer：")
          .replace(/^\s*(ANSWER[:：])\s*/, "答案Answer：");

        var m = block.match(/^\s*(答案\S+?[:：])\s*(.*?)\s*(?:\n|$)/);
        if (!m) return undefined;
        var answer = ["answer"];
        var n;
        if (qt_type == "1") {
      ...
      qtAnalysis: function qtAnalysis(block, next) {
        block = block
          .replace(/^\s*【\s*解析\s*】\s*/, "：")
          .replace(/^\s*(解析[:：])\s*/, "解析Analysis：")
          .replace(/^\s*(analysis[:：])\s*/, "解析Analysis：")
          .replace(/^\s*(Analysis[:：])\s*/, "解析Analysis：")
          .replace(/^\s*(ANALYSIS[:：])\s*/, "解析Analysis：");
        var m = block.match(/^\s*(解析\S+?[:：])\s*(.*?)\s*(?:\n|$)/);
        if (!m) return undefined;
      ...
      qtDifficult: function qtDifficult(block, next) {
        block = block
          .replace(/^\s*【\s*难度\s*】\s*/, "难度Difficulty：")
          .replace(/^\s*(难度[:：])\s*/, "难度Difficulty：")
          .replace(/^\s*(answer[:：])\s*/, "难度Difficulty：")
          .replace(/^\s*(Difficulty[:：])\s*/, "难度Difficulty：")
          .replace(/^\s*(DIFFICULTY[:：])\s*/, "难度Difficulty：");
        var m = block.match(/^\s*(难度\S+?[:：])\s*(.*?)\s*(?:\n|$)/);
        if (!m) return undefined;
        ...
        console.log(difficult, "kkpk");
        return [difficult];
      },
      qtChapter: function qtChapter(block, next) {
        block = block
          .replace(/^\s*【\s*章节\s*】\s*/, "章节Chapter：")
          .replace(/^\s*(章节[:：])\s*/, "章节Chapter：")
          .replace(/^\s*(chapter[:：])\s*/, "章节Chapter：")
          .replace(/^\s*(Chapter[:：])\s*/, "章节Chapter：")
          .replace(/^\s*(CHAPTER[:：])\s*/, "章节Chapter：");
        var m = block.match(/^\s*(章节\S+?[:：])\s*(.*?)\s*(?:\n|$)/);
        if (!m) return undefined;
        var chapter = ["chapter"];
        Array.prototype.push.apply(chapter, this.processInline(m[0]));
        ...
        console.log(chapter, "kkpk");
        return [chapter];
      },
      qtKnowLedge: function qtKnowLedge(block, next) {
        block = block
          .replace(/^\s*【\s*知识点\s*】\s*/, "知识点Knowledge：")
          .replace(/^\s*(知识点[:：])\s*/, "知识点Knowledge：")
          .replace(/^\s*(knowledge[:：])\s*/, "知识点Knowledge：")
          .replace(/^\s*(Knowledge[:：])\s*/, "知识点Knowledge：")
          .replace(/^\s*(KNOWLEDGE[:：])\s*/, "知识点Knowledge：");
        var m = block.match(/^\s*(知识点\S+?[:：])\s*(.*?)\s*(?:\n|$)/);
        if (!m) return undefined;
        var knowledge = ["knowledge"];
        Array.prototype.push.apply(knowledge, this.processInline(m[0]));
        ...
        console.log(knowledge, "kkpk");
        return [knowledge];
      },
      qtIndicator: function qtIndicator(block, next) {
        block = block
          .replace(/^\s*【\s*学科素养\s*】\s*/, "学科素养Indicator：")
          .replace(/^\s*(学科素养[:：])\s*/, "学科素养Indicator：")
          .replace(/^\s*(Indicator[:：])\s*/, "学科素养Indicator：");
        var m = block.match(/^\s*(学科素养\S+?[:：])\s*(.*?)\s*(?:\n|$)/);
        if (!m) return undefined;
        var indicator = ["indicator"];
        Array.prototype.push.apply(indicator, this.processInline(m[0]));
      ...
      qtOrder: function qtOrder(block, next) {
        block = block
          .replace(/^\s*【\s*答案是否正序\s*】\s*/, "：")
          .replace(/^\s*(答案是否正序[:：])\s*/, "答案是否正序：")
          .replace(/^\s*(答案是否正序[:：])\s*/, "答案是否正序：")
          .replace(/^\s*(答案是否正序[:：])\s*/, "答案是否正序：")
          .replace(/^\s*(答案是否正序[:：])\s*/, "答案是否正序：");
        var m = block.match(/^\s*(答案是否正序\S+?[:：])\s*(.*?)\s*(?:\n|$)/);
        console.log(m, "mm");
        if (!m) return undefined;
        var order = ["order"];
        Array.prototype.push.apply(order, this.processInline(m[0]));
  ...
  expose.renderJsonML = function (jsonml, options) {
    ...
    function regStr(string) {
      ...
      var qt_chapter = new RegExp("qt_chapter");
      var qt_knowledge = new RegExp("qt_knowledge");
      var qt_indicator = new RegExp("qt_indicator");
      ...
      insert.chapter = qt_chapter.test(string) ? string : insert.chapter;
      insert.knowledge = qt_knowledge.test(string) ? string : insert.knowledge;
      insert.indicator = qt_indicator.test(string) ? string : insert.indicator;
      insert.label = qt_label.test(string) ? string : insert.label;
      insert.comKeyWord = qt_comKeyWord.test(string)
        ? string
        : insert.comKeyWord;
      insert.coreKeyWord = qt_coreKeyWord.test(string)
        ? string
        : insert.coreKeyWord;
      ...
      status.qt_difficult = qt_difficult.test(string)
        ? true
        : status.qt_difficult;
      status.qt_chapter = qt_chapter.test(string) ? true : status.qt_chapter;
      status.qt_knowledge = qt_knowledge.test(string)
        ? true
        : status.qt_knowledge;
      status.qt_indicator = qt_indicator.test(string)
        ? true
        : status.qt_indicator;
      status.qt_label = qt_label.test(string) ? true : status.qt_label;

      status.qt_comKeyWord = qt_comKeyWord.test(string)
        ? true
        : status.qt_comKeyWord;
      status.qt_coreKeyWord = qt_coreKeyWord.test(string)
        ? true
        : status.qt_coreKeyWord;
    ...
    for (var i = 0; i < content.length; i++) {
      ...
      for (var j = i; j < content.length - 1; j++) {
        //都是检测下一项，所以不用再去对最后一行做判断
        if (error_reg.test(content[j + 1])) {
          //检测当前项的下一项，若为error则与当前项拼接
          //当下一项中有图片（fr-dib是froala编辑器中图片的class）；并且当前项为答案时
          if (
            content[j + 1].match('class="fr-dib"') &&
            content[j].match("qt_answer")
          ) {
            //判断题、填空题答案不能有图片
            ...
            //问答题的答案中不能只放图片(问答题；字符串长度小于4；答案；下一项是图片)
              ...
              var answerLength = $(content[j])
                .text()
                .replace(/\s*/g, "").length; //答案长度
              if (answerLength < 4) {
                content[j] = content[j].replace(
                  /qt_answer/,
                  "qt_answer qt_error",
          ...
          content[j + 1] = (content[j] + content[j + 1]).replace(
            /<\/p><p class='error'>/,
      ...
      if (params.count > 1) {
        ...
        console.log(insert, "inin");
        if (
          $(insert.difficult).text().trim().slice(13).trim() != "简单" &&
          $(insert.difficult).text().trim().slice(13).trim() != "普通" &&
          $(insert.difficult).text().trim().slice(13).trim() != "困难"
        ) {
          var diffDivTxt = $(".simulationSelect.difficult .title-font").text();
          if (diffDivTxt != "") {
            insert.difficult =
              '<p class="qt_difficult qt_error_diff"><span class="title">难度Difficulty：</span>' +
              diffDivTxt +
              "</p>";
            insert.left =
              "<div class='question check_error_diff' data-type='" +
              qt_type +
              "'>";
          }
        }
        //如果单选题、多选题的答案中有图片
        if (
          insert.answer.match('class="fr-dib"') &&
          (qt_type == "1" || qt_type == "2")
        ) {
        ...
        if (qt_type == "5") {
          var characterCheck =
            /[`~!@#$%^&*()_\-+=<>?:"{},.\/;'\\[\]·~！@#￥%……&*（）——\-+={}《》？：“”【】；‘’，。]/im; //解决特殊字符判分不准的问题
          var _comKeyWord = $(insert.comKeyWord)
            .text()
            .substring(6, $(insert.comKeyWord).text().length);
          var _coreKeyWord = $(insert.coreKeyWord)
            .text()
            .substring(6, $(insert.coreKeyWord).text().length);
          if (
            characterCheck.test(_comKeyWord) ||
            characterCheck.test(_coreKeyWord)
          ) {
    ...
    switch (tag) {
      case "title":
        ...
        return (
          ...
          content
            ...
            .replace(
              /^\s*([0-9]+\.)|(((\()|（)[0-9]+((\))|）))/,
              "<span class='type-box'><span class='title type-name-" +
                qt_type +
                "'>$1$2</span><span class='type-name type-name-" +
                qt_type +
                "'>" +
                typeName[Number(qt_type) - 1] +
                "</span>" +
                span +
                "</span>",
            ) +
      ...
      case "keyA":
        ...
        return (
          ...
          ">" +
          checkOrRadio +
          content.join("").replace("A.", "<span class='title'>A.</span>") +
          "</p>"
        );
        break;
      case "keyB":
        ...
        return (
          ...
          ">" +
          checkOrRadio +
          content.join("").replace("B.", "<span class='title'>B.</span>") +
          "</p>"
        );
        break;
      case "keyC":
        ...
        return (
          ...
          ">" +
          checkOrRadio +
          content.join("").replace("C.", "<span class='title'>C.</span>") +
          "</p>"
        );
        break;
      case "keyD":
        ...
        return (
          ...
          ">" +
          checkOrRadio +
          content.join("").replace("D.", "<span class='title'>D.</span>") +
          "</p>"
        );
        break;
      case "keyE":
        ...
        return (
          ...
          ">" +
          checkOrRadio +
          content.join("").replace("E.", "<span class='title'>E.</span>") +
          "</p>"
        );
        break;
      case "keyF":
        ...
        return (
          ...
          ">" +
          checkOrRadio +
          content.join("").replace("F.", "<span class='title'>F.</span>") +
          "</p>"
        );
        break;
      case "keyG":
        ...
        return (
          ...
          ">" +
          checkOrRadio +
          content.join("").replace("G.", "<span class='title'>G.</span>") +
          "</p>"
        );
        break;
      case "keyH":
        ...
        return (
          ...
          ">" +
          checkOrRadio +
          content.join("").replace("H.", "<span class='title'>H.</span>") +
          "</p>"
        );
        break;
      case "answer":
        ...
        if (qt_type == "1" || qt_type == "2") {
          return (
            ...
            content
              ...
              .replace(
                /(&nbsp;)*\s*答案Answer[:：]/,
                "<span class='title'>答案Answer：</span>",
              ) +
            "</p>"
          );
        } else {
      ...
      case "analysis":
        ...
        return (
          ...
          content
            ...
            .replace(
              /(&nbsp;)*解析\S+?[:：]/,
              "<span class='title'>解析Analysis：</span>",
            ) +
          "</p>"
        );
        break;
      case "difficult":
        ...
        return (
          ...
          content
            ...
            .replace(
              /(&nbsp;)*难度\S+?[:：]/,
              "<span class='title'>难度Difficulty：</span>",
            ) +
          "</p>"
        );
      case "chapter":
        tag_attrs = " class='qt_chapter'";
        return (
          "<p" +
          tag_attrs +
          ">" +
          content
            ...
            .replace(
              /(&nbsp;)*章节\S+?[:：]/,
              "<span class='title'>章节Chapter：</span>",
            ) +
          "</p>"
        );
        break;
      case "knowledge":
        tag_attrs = " class='qt_knowledge'";
        return (
          "<p" +
          tag_attrs +
          ">" +
          content
            ...
            .replace(
              /(&nbsp;)*知识点\S+?[:：]/,
              "<span class='title'>知识点Knowledge：</span>",
            ) +
          "</p>"
        );
        break;
      case "indicator":
        tag_attrs = " class='qt_indicator'";
        return (
          "<p" +
          tag_attrs +
          ">" +
          content
            ...
            .replace(
              /(&nbsp;)*学科素养\S+?[:：]/,
              "<span class='title'>学科素养Indicator：</span>",
            ) +
          "</p>"
        );
        break;
      ...
      default:
        ...
        return (
          "<" + tag + tag_attrs + ">" + content.join("") + "</" + tag + ">"
        );
    }
  }
